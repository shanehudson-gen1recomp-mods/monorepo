...
|---|---|---|
| [Dramatic Shape Voxel Mod](https://github.com/DramaticShape/DramaticShapeVoxelMod) | 1.6.2 | built-in scene detection plus the adapter composing both battlers into its billboard textures; STADIUM pair models covered by the headless suite against the verified module contract, not yet play-tested (partner placement direction in particular needs an in-game look) |
| Dramaless Shape | 1.6.4 | same Stadium surface as Dramatic Shape (plus `hit`/`attachment`, unused here); same suite coverage, not yet play-tested |
| Battle Art voxel fork | 1.7.6 | same scene detection path as Dramatic Shape; carries no Stadium family, so doubles stay on the composed cards |
