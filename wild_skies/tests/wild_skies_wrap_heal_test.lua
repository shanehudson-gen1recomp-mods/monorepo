...
-- The shared draw watchdog must re-arm the wrap; a chain a foreign
-- restore leaves holding two of our wraps must tick exactly once per
-- frame; and the shared tag must keep a second family mod's watchdog
-- from rewrapping every frame (the free_fly fight).
...
-- the tag matches, so drawn frames must never grow the chain: this is
-- the two-watchdog fight that broke free_fly
local settled = OC.update
OC.draw({}); OC.draw({}); OC.draw({})
